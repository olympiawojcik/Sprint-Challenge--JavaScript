...
for (let i = 0; i<graduates.length; i++) {
  contact = `${graduates[i].first_name} ${graduates[i].email}`;
...
for (let i = 0; i<universities.length;i++) {
  //the String includes() method determines whether one string may be found within a nother string, returning true or false.
...
//   display = `Name: ${item.animal_name}, Scientific: ${item.scientific_name}`
//   animalNames.push(display);
// })

//CONVERT TO ARROW
const displayCB = item => {
  display = `Name: ${item.animal_name}, Scientific: ${item.scientific_name}`
  animalNames.push(display);
}

zooAnimals.forEach(displayCB);
...
//CONVERT TO ARROW
const lowerCaseCB = item => {
  return item.animal_name.toLowerCase();
}

const lowerCase = zooAnimals.map(lowerCaseCB);
...
const largerPopulation = zooAnimals.filter(filterCB);
...
//REGULAR WAY
// const populationTotal = zooAnimals.reduce(function(sum, currentVal){
//   return sum + currentVal.population;
// },0)

//CONVERT TO ARROW
const reduceCB = (sum, currentVal) => {
  return sum + currentVal.population;
}

const populationTotal = zooAnimals.reduce(reduceCB, 0);
